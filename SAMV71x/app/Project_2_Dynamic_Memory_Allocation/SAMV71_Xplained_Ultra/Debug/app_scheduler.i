...
# 41 "C:\\SAMV7x\\SAMV71x\\hal\\libchip_samv7\\include\\samv7/system_samv71.h"
...
# 42 "C:\\SAMV7x\\SAMV71x\\hal\\libchip_samv7\\include\\samv7/system_samv71.h" 2
...
__attribute__ ((aligned (1))) uint8_t priorityScheduler_Counter ;
__attribute__ ((aligned (1))) uint8_t gpriorityScheduler_Counter ;
...
static const Pin sw0 = {(1u << 9), ((Pio *)0x400E0E00U), (10), 4, (1 << 0) | (1 << 3) | (0 | (1 << 6) | (1 << 4))};

static void _Button0_Handler( const Pin* pPin )
{
 if ( pPin == &sw0 )
    {
  if(gpriorityScheduler_Counter == 5)
        {
            gpriorityScheduler_Counter = 1;
        }
        else
        {
            gpriorityScheduler_Counter++;
        }
 }
}
...
void _ConfigureButtons( void )
{
 ...
 PIO_SetDebounceFilter( &sw0, 10 ) ;



 PIO_ConfigureIt( &sw0, _Button0_Handler ) ;



 NVIC_EnableIRQ( (IRQn_Type)sw0.id ) ;
...
void vfnSchedulePoint( tSchedulingTask * Task )
{
    if(Task->u8Priority < TimeTriggeredTasks[TASKS_PB_MS].u8Priority)
    {
        TasksScheduler_Task_ID_Backup = TimeTriggeredTasks[TaskScheduler_Task_ID_Running].enTaskState;
        TaskScheduler_Task_ID_Running = TimeTriggeredTasks[TASKS_PB_MS].TaskId;

        TimeTriggeredTasks[TASKS_PB_MS].enTaskState = RUNNING;


        TimeTriggeredTasks[TASKS_PB_MS].ptrTask();

        TimeTriggeredTasks[TASKS_PB_MS].enTaskState = SUSPENDED;

        TaskScheduler_Task_ID_Running = TasksScheduler_Task_ID_Backup;
    }
    else
    {
